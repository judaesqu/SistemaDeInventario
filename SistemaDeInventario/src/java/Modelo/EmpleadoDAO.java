...
public class EmpleadoDAO {
    ...
    public Empleado validar(String User, String Password){
        ...
        String sql = "SELECT * FROM empleado WHERE User=? AND Password=MD5(?)";
        try{
            con=cn.conector();
    ...
    public int agregar (Empleado em){
        String sql = "INSERT INTO empleado (Nombre_empleado, Cargo, Fecha_contratacion, User, Password, Email) VALUES (?,?,?,?,MD5(?),?)";
        try{
            con=cn.conector();
            ps=con.prepareStatement(sql);
